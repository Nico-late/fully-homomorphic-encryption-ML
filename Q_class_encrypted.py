...
class Q_class_encrypted(object):
    def __init__(self, Q, input_type='int'):
        ...
        if not os.path.exists('mlp_encrypted_weights'):
            os.mkdir('mlp_encrypted_weights')

        for operation in self.operations:

            # Create operations sub-directories if don't exist
            if not os.path.exists('mlp_encrypted_weights/{}'.format(operation)):
                os.mkdir('mlp_encrypted_weights/{}'.format(operation))

            # Load or create MLP model for each operation
            self.mlp[operation] = MLP_encrypted(layers=[2, 10, 2], activations=['relu', 'linear'], Q=Q)
            path = 'mlp_encrypted_weights/{}/{}_Q={}_2_10_2.model'.format(operation,input_type,Q)
